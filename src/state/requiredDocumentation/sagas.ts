...
import { addDocumentationAction, deleteDocumentationAction, updateDocumentationAction } from '../user/actions';
...
function* newDocStartAsync({ payload }: NewDocStart): Generator<unknown, void, Documentation> {
  try {
...
function* updateStartAsync({ payload }: UpdateStart): Generator<unknown, void, Documentation> {
  try {
    const form = toFormData(payload);
...
function* deleteStartAsync({ payload }: DeleteStart): Generator<unknown, void, Documentation> {
  try {
...
export default function* sagas() {
  yield all([call(fetchStart), call(newDocStart), call(updateStart), call(deleteStart)]);
}
