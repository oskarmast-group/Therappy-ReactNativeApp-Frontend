...
const Profile: React.FC = () => {
  ...
  useEffect(() => {
    if (user.current) {
      ...
      if (user.current.extraData && user.current.userType === UserType.THERAPIST) {
        setTherapistData({
  ...
  const onChangeTherapist = (key: keyof UpdateTherapistFields) => (value: string) => {
        ...
        {user.fetching.fetch.isFetching || !user.current ? (
          ...
          <View style={styles.container}>
            ...
            <Input
              ...
              editable={true}
              loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'name'}
            ...
            <Input
              ...
              editable={true}
              loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'lastName'}
            ...
            <InputCalendar
              ...
              loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'dob'}
            ...
            {user.current?.userType === UserType.THERAPIST && (
              <>
                <Input
                  ...
                  editable={true}
                  loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'title'}
                ...
                <Input
                  ...
                  editable={true}
                  loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'phrase'}
                  onSubmit={() => onSubmitTherapist('phrase')}
                />
                <Input
                  inputProps={{
                    value: therapistData.experience,
                    onChangeText: (value) => onChangeTherapist('experience')(value),
                    numberOfLines: 5,
                    multiline: true,
                    textAlignVertical: 'top',
                  }}
                  ...
                  editable={true}
                  loading={user.fetching.update.isFetching && user.fetching.update.config?.key === 'experience'}
