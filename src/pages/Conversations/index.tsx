...
const Conversation: React.FC = () => {
  ...
  }, [conversationsDispatcher, userDispatcher, messagesDispatcher, conversationId]);
  ...
  const user = useMemo(
    () => (conversations.conversation?.users ? conversations.conversation.users[0] : null),
  ...
  return (
    <MainContainer withSideMenu={false} withBottomNavigation={false}>
      <TopBar
        title={`${
          user?.userType === UserType.THERAPIST ? (user?.title ? `${user?.title} ` : '') : ''
        }${user?.name ?? ''}`}
        fontSize={20}
        color={DARKER_TEXT}
      />
