...
const Calendar: React.FC = () => {
  ...
  const list = useMemo(() => {
    ...
    if (page === 0) {
      const appointmentsList = appointments.list.filter(({ date }) => new Date(date) > sub(new Date(), { hours: 1 }));
  ...
  return (
    <Scrollable>
      ...
      <View style={styles.tabContainer}>
        <TouchableOpacity
          style={page === 0 ? StyleSheet.compose(styles.tab, styles.tabActive) : styles.tab}
          onPress={() => setPage(0)}
        >
          <BaseText color={page === 0 ? 'white' : PRIMARY_GREEN} textAlign={'center'}>
            Próximas
          </BaseText>
        </TouchableOpacity>
        <TouchableOpacity
          style={page === 1 ? StyleSheet.compose(styles.tab, styles.tabActive) : styles.tab}
          onPress={() => setPage(1)}
        >
          <BaseText color={page === 1 ? 'white' : PRIMARY_GREEN} textAlign={'center'}>
            Pasadas
          </BaseText>
        </TouchableOpacity>
      </View>
