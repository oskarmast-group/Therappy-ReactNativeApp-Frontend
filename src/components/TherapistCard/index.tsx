...
const TherapistCard: React.FC<{
  ...
  const { id, title, name, lastName, profileImg, reviewAvg, reviewsCount } = therapist;
  return (
    <View style={withBorder ? StyleSheet.compose(styles.container, styles.containerBorder) : styles.container}>
      <LinkContainer id={id} shouldRender={clickable}>
        <ImageContainer width={imageProps?.width} height={imageProps?.height} borderRadius={imageProps?.borderRadius}>
          {profileImg ? (
            <ImageComponent
        ...
        <View style={styles.informationContainer}>
          <BaseText fontSize={18} marginBottom={5}>{`${title ?? ''} ${name} ${lastName}`}</BaseText>
